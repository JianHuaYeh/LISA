 ...
 * @author jhyeh
 */
public class CalcRecommendationSparse {
    private String training;
    private String testing;
    private int method;
    private SparseMatrix rawdata, data;
    private int umax;
    private int umin;
    private int tmax;
    private int tmin;
    ...
    public static void main(String[] args) {
        ...
        CalcRecommendationSparse nr = new CalcRecommendationSparse(args[0], args[1],
                args[2], m);
        nr.go();
    }

    public CalcRecommendationSparse(String raw, String tr, String ts, int m) {
        this.training = tr;
        this.testing = ts;
        this.method = m;
    ...
    public double doPredict(int uid, int tid) {
        ...
        for (int i=0; i<rows; i++) {
            ...
            double[] vec0 = this.data.getRow(uid-umin);
            //double[] vec1 = this.data[i]; // i = uid2-umin
            double[] vec1 = this.data.getRow(i); // i = uid2-umin

            double rating = 0.0;
            try {
                //rating = this.rawdata[i][tid-tmin];
                rating = this.rawdata.get(i, tid-tmin);
            } catch (Exception e) {
