 ...
 * @author jhyeh
 */
public class FCMClusteringSparse {
    private SparseMatrix data;
    private SparseMatrix weight;
    private SparseMatrix centers;
    private int dim;
    private int k;
    private int cols;
    private int method;
    ...
    public static void main(String[] args) throws Exception {
        ...
        FCMClusteringSparse fcm = new FCMClusteringSparse(args[0], r, m);
        fcm.doClustering();
        fcm.closestHard();
    }
    ...
    public void doClustering() {
        ...
        if(Double.isNaN(weight.get(0, 0))) System.err.println("init");
        //if(Double.isNaN(weight[0][0])) System.err.print("init weight is NaN");
        if(Double.isNaN(weight.get(0, 0))) System.err.print("init weight is NaN");
        ...
        for(int t=0; t<120; t++){
            ...
            if((amplitude[0]=Math.abs(fcm[0]-fcm[1]))>0.025){
                ...
                if(Double.isNaN(weight.get(0, 0)))
    ...
    private void init(){
        ...
        weight = new SparseMatrix(this.cols, this.k);
        double one;
        for(int i=0;i<this.cols;i++){
            one=1;
            for(int j=0;j<this.k-1;j++){
                //weight[i][j]=Math.random()%one;
                weight.put(i, j, Math.random()%one);
                //one-=weight[i][j];
                one -= weight.get(i, j);
            }
            //weight[i][this.k-1]=one;
            weight.put(i, this.k-1, one);
        }
    }
    ...
    private void centers(){
        ...
        centers = new SparseMatrix(k, this.dim);
        double[] sum1,sum2;
        double temp;
        for(int i=0; i<k; i++){
    ...
    private double JFCM(){
        ...
        for(int i=0;i<k;i++){
            for(int j=0;j<this.cols;j++){
                ...
                fcm += Math.pow(weight.get(j, i), 2.0)*
                       Math.pow(Math.abs(distance(data.getRow(j),centers.getRow(i),this.method)),2.0);
            }
        }
        return fcm;
    }
        ...
        for(int l=0; l<k; l++){
            for(int i=0; i<this.cols; i++){
                ...
                for(int j=0; j<k; j++){
                    ...
                    if(Math.abs(distance(data.getRow(i),centers.getRow(j),this.method))==0)
                        temp=temp+Math.pow(Math.abs(distance(data.getRow(i),centers.getRow(l),this.method))/1, 2.0);
                    else
                        temp=temp+Math.pow(Math.abs(distance(data.getRow(i),centers.getRow(l),this.method))/Math.abs(distance(data.getRow(i),centers.getRow(j),this.method)), 2.0);
                    if(Double.isNaN(temp))
                        System.err.println("resetWeight temp is NaN\ttemp="+Math.abs(distance(data.getRow(i),centers.getRow(l),this.method))+"/"+Math.abs(distance(data.getRow(i),centers.getRow(j),this.method)));
                }
                ...
                if(temp == 0){
                    ...
                    for(int x=1; x<k; x++) weight.put(i, x, 0);
                }
                //else weight[i][l]=1/temp;
                else weight.put(i, l, 1/temp);
            }
        }
    }
    ...
    public void closestHard(){
        ...
        //for(int i=0;i<weight.length;i++){
        for(int i=0;i<weight.rowDim();i++){
            index=-1;max=Double.MIN_VALUE;
            //for(int j=0;j<weight[i].length;j++)
            for(int j=0;j<weight.colDim();j++)
                //if(weight[i][j]>max){index=j;max=weight[i][j];}
                if(weight.get(i, j)>max){index=j;max=weight.get(i, j);}
            ...
            try{
                ...
                for(int x=0;x<weight.rowDim();x++){
                    //for(int y = 0; y<weight[0].length;y++)
                    for(int y = 0; y<weight.colDim();y++)
                        //System.err.print(weight[x][y]+",");
                        System.err.print(weight.get(x, y)+",");
                    System.err.println();
                }
            }
            // set.add(blog);
    ...
    public FCMClusteringSparse(String s, double r, int m) {
        this.ratio = r;
        this.method = m;
